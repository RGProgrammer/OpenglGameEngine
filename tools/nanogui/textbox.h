...
public:
    ...
    TextBox(Widget *parent, const string &value = "Untitled");
    ...
    const string &placeholder() const { return mPlaceholder; }
    /// Specify a placeholder text to be displayed while the text box is empty.
    void setPlaceholder(const string &placeholder) { mPlaceholder = placeholder; }
    ...
    void setCallback(const function<bool(const string& str)> &callback) { mCallback = callback; }
...
protected:
    bool checkFormat(const string& input,const string& format);
    bool copySelection();
...
public:
    IntBox(Widget *parent, Scalar value = (Scalar) 0) : TextBox(parent) {
        ...
        setValueIncrement(1);
        setMinMaxValues(numeric_limits<Scalar>::lowest(), numeric_limits<Scalar>::max());
        setValue(value);
        setSpinnable(false);
    }

    Scalar value() const {
        istringstream iss(TextBox::value());
        Scalar value = 0;
        iss >> value;
        return value;
    }
    ...
    void setValue(Scalar value) {
        Scalar clampedValue = min(max(value, mMinValue),mMaxValue);
        TextBox::setValue(to_string(clampedValue));
    }

    void setCallback(const function<void(Scalar)> &cb) {
        TextBox::setCallback(
...
public:
    ...
    Scalar value() const {
        return (Scalar) stod(TextBox::value());
    }
    ...
    void setValue(Scalar value) {
        ...
        Scalar clampedValue = min(max(value, mMinValue),mMaxValue);
        char buffer[50];
        snprintf(buffer, 50, mNumberFormat.c_str(), clampedValue);
