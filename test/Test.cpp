...
void RGP_CORE::Test::Start() {
	...
	if (!testmodel1->LoadModelFromFile("..//Resources//sky.obj"))
		printf("error \n");
	else
    ...
    while(true){
		...
		state = glfwGetKey(m_Renderer->getTarget()->getglfwWindow(), GLFW_KEY_P);
		if (state == GLFW_PRESS) {
			m_Renderer->setEmissionIntensity(m_Renderer->getEmissionIntensity() + 0.02);
		}
		state = glfwGetKey(m_Renderer->getTarget()->getglfwWindow(), GLFW_KEY_O);
		if (state == GLFW_PRESS) {
			m_Renderer->setEmissionIntensity(m_Renderer->getEmissionIntensity() - 0.02);
		}
