...
namespace RGP_CORE{
	...
	public:
		...
		~GLShaderProgramsManager();
		void Destroy();
		_u32b CreateProgramFromOneFile(const _s8b* Filename);
		_u32b CreateProgramFromFiles(const _s8b* VertexFile,const  _s8b* FrgamentFile=NULL);
		_u32b CreateProgramFromFiles(const _s8b* VertexFile,const _s8b* GeometryFile,const _s8b* FrgamentFile);
		void  BindProgram(_u32b ID);
		void  DeleteProgram(_u32b ID);
	private:
		_u32b LoadShaderBuffer(GLenum type, const _s8b* Buffer, int buffersize);
		_u32b LoadShaderFile(GLenum Type, const _s8b* filename);
		_u32b CreateGLProgram(const _s8b* VertexFile,const _s8b* GeometryFile,const _s8b* FrgamentFile);
		_u32b doExiste(const _s8b* VertexFile,const  _s8b* GeometryFile,const _s8b* FrgamentFile);// verify if the shader program existeif it is returns the progran ID
	private:
	...
	public:
		...
		void    setEmissionIntensity(_float value);
		_float    getEmissionIntensity();

		// material
		_u32b			CreateMaterial(const Material* material); //return material index (>=1)
        ...
        _u32b  CreateGLProgramFromFile(const  _s8b* VertexFile,const _s8b* FragmentFile=NULL);
		_u32b  CreateGLProgramFromFile(const _s8b* VertexFile,const _s8b* GeometryFile,const  _s8b* FragmentFile);
